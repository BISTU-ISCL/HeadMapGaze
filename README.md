...
- 若仍未被 Qt Creator / Designer 识别，请确认：插件是用与目标 Designer 相同的 Qt 5.15.2 套件编译的；并在 Qt Creator 的**帮助 -> 关于插件**中检查 `Designer` 分组是否列出了 `heatmapoverlaywidgetplugin`，必要时清理旧版插件后重新复制最新构建的动态库。
